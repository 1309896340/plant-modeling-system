...
public:
  ...
  void pushGeometry(shared_ptr<Geometry> mesh,
  ...
  virtual void update() {
    ...
    for (uint32_t i = 0; i < num; i++) {
      ...
      meshes.emplace_back(make_shared<FixedGeometry>(
          mesh_vertices, this->goemetries[i]->getSurfaces()));
    }
  ...
  static shared_ptr<Composition> Cube(float xWidth, float yWidth, float zWidth,
  ...
  static shared_ptr<Composition> Cylinder(float radius, float height,
  ...
  static shared_ptr<Composition> Cone(float radius, float height, uint32_t PNum,
  ...
  static shared_ptr<Composition> Arrow(float radius, float length) {
    ...
    auto body = Composition::Cylinder(radius * radiusRatio, length * bodyRatio,
                                      16, 1, 2);
