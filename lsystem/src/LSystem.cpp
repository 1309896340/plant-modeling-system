  ...
  for (uint32_t i = 0; i < symSeq->syms.size(); i++) {
    ...
    case LSysConfig::Sym::NORMAL: {
      auto prod_iter = find_if(
          this->lsystem->productions.begin(), this->lsystem->productions.end(),
...
string LSystem::LSystem::getCurrent() const { return this->current; }
...
void LSystem::LSystem::reset() { this->current = this->axiom; }