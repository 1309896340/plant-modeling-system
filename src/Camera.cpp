...
void Camera::surround(float dx, float dy) {
  ...
  glm::vec3 new_pos;
  new_pos.x = this->surround_info.distance * sin(this->surround_info.theta) *
              cos(this->surround_info.phi);
  new_pos.y = this->surround_info.distance * cos(this->surround_info.theta);
  new_pos.z = this->surround_info.distance * sin(this->surround_info.theta) *
              sin(this->surround_info.phi);
...
void Camera::record() {
  ...
  this->surround_info.theta =
      acos(glm::dot(glm::normalize(dist_v), glm::vec3(0.0f, 1.0f, 0.0f)));
  this->surround_info.phi = atan2(dist_v.z, dist_v.x);
}

// 所有通过修改theta_s和phi_s意图更新theta和phi从而更改toward的操作
// 由于这两个值是角度值，因此需要考虑中间转换
// 另外，这两个值需要在修改之后立即更新，而不能等到一个render循环的末尾惰性更新，因此必须保证调用方在修改后立即调用
// updateAttitudeFromShadow()
// 也正因为这需要即时的手动更新，所以不需要记录flag来延迟更新
// 由于范围是角度制，因此实际返回的是attitude shadow，所以调用的是
// updateAttitudeFromShadow() 而不是 updateToward()
float &Camera::getTheta() { return this->theta_s; }
float &Camera::getPhi() { return this->phi_s; }
...
void Camera::apply_view_done() { this->view_is_changed = false; }
void Camera::apply_projection_done() { this->project_is_changed = false; }

bool Camera::isProjectionChanged() const { return this->project_is_changed; }
bool Camera::isViewChanged() const { return this->view_is_changed; }
