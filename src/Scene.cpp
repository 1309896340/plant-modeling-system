...
string Component::GeometryObject::getName() const { return this->name; }
...
void Scene::Scene::init_scene_obj() {
  ...
  shared_ptr<Component::GeometryObject> obj2 =
      make_shared<Component::GeometryObject>("Axis", axis);
  ...
  shared_ptr<Component::GeometryObject> cursor_obj =
  ...
  shared_ptr<Component::GeometryObject> obj3 =
      make_shared<Component::GeometryObject>(
  ...
  shared_ptr<Component::GeometryObject> side_left_obj =
      make_shared<Component::GeometryObject>(
          "Side_left", side_left,
  ...
  shared_ptr<Component::GeometryObject> side_back_obj =
      make_shared<Component::GeometryObject>(
          "Side_back", side_back,
  ...
  shared_ptr<Component::GeometryObject> side_top_obj =
      make_shared<Component::GeometryObject>(
          "Side_top", side_top,
...
void Scene::Scene::init_skybox() {
  ...
  for (int i = 0; i < skybox_texture_names.size(); i++) {
    ...
    glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_RGBA,
                 this->cubemaps[i].width, this->cubemaps[i].height, 0, GL_RGBA,
                 GL_UNSIGNED_BYTE, this->cubemaps[i].img);
...
void Scene::Scene::showGround() {
  shared_ptr<Component::GeometryObject> ptr =
      findGeometryObjectByName("Ground");
  ptr->status.visible = true;
}
...
void Scene::Scene::printRadiosityInfo() {
  ...
  for (auto &[cur_obj, context] : this->objs) {
    ...
    printf("%s : (%.2f, %.2f, %.2f)\n", cur_obj->getName().c_str(), flux_sum.r,
...
Scene::Scene::findGeometryObjectByName(const string &name) {
  auto iter = find_if(this->objs.begin(), this->objs.end(),
                      [&](pair<const shared_ptr<Component::GeometryObject>,
                               unique_ptr<GeometryContext>> &obj_pair) {
...
glm::vec3 Scene::Scene::screen2world(glm::vec2 pos) {
  ...
  glm::vec4 target_dir = glm::vec4(
      glm::normalize(glm::vec3(pos.x * near * tanf(fov / 2.0f) * aspect,
...
bool Scene::Scene::imgui_menu() {
  ...
  if (ImGui::TreeNodeEx(TEXT("相机"), ImGuiTreeNodeFlags_Selected |
    ...
    ImGui::DragFloat3(TEXT("位置"), glm::value_ptr(this->camera.getPosition()),
                      0.05f, -FLT_MAX, FLT_MAX);
  ...
  if (ImGui::TreeNodeEx(TEXT("光源"), ImGuiTreeNodeFlags_DefaultOpen)) {
    ...
    // -20.0f, 20.f);
    ImGui::DragFloat3(TEXT("位置"), glm::value_ptr(this->light.position), 0.05f,
                      -FLT_MAX, FLT_MAX);
    // 暂时这么写
    shared_ptr<Component::GeometryObject> ptr =
        findGeometryObjectByName("Light");
    if (ptr)
  ...
  if (ImGui::TreeNodeEx(TEXT("几何管理"), ImGuiTreeNodeFlags_DefaultOpen)) {
    ...
    if (!imgui.list_items.empty() && this->imgui.cur != nullptr &&
      ...
      if (ImGui::DragFloat3(
              TEXT("位置"),
              glm::value_ptr(this->imgui.cur->transform.getPosition()), 0.05f,
              -FLT_MAX, FLT_MAX)) {
        // 更新包围盒以及其渲染缓冲
      ...
      ImGui::DragFloat4(
          TEXT("姿态"),
          glm::value_ptr(this->imgui.cur->transform.getAttitude()), 0.05f,
          -FLT_MAX, FLT_MAX);
      ...
      for (auto &[name, arg_val] : imgui.cur->getGeometry()->parameters)
        if (arg_val->isTopo())
          std::visit(
              [name, this, arg_val](auto &&arg) {
                using T = decay_t<decltype(arg)>;
                auto [min_v, max_v] = arg_val->getRegion();
                float step = (min_v == -FLT_MAX || max_v == FLT_MAX)
                                 ? 1.0f
                                 : (max_v - min_v) / 100.0f;
                if constexpr (is_same_v<T, float>) {

                  if (ImGui::DragFloat(name.c_str(), &arg, step, min_v,
                                       max_v)) {
                  ...
                  if (ImGui::DragInt(
                          name.c_str(), reinterpret_cast<int *>(&arg),
                          static_cast<int>(step), static_cast<int>(min_v),
  ...
  if (ImGui::TreeNodeEx(TEXT("调试"))) {
    ...
    ImGui::Checkbox(TEXT("显示BVH包围盒"), &this->isShowBvhFrame);
  ...
  //     throw
  //     runtime_error("Scene::imgui_menu()中，更新this->lsystem的axiom！");
  ...
  //     this->add("skeleton", this->lsystem.skeleton, Transform{glm::vec3(0.5f,
  //     0.03f, 0.5f)}); this->lsystem.iter_n++;
  //   }
  //   ImGui::SameLine();
...
void Scene::Scene::loadIcon() {
  GLFWimage image;
  image.pixels = stbi_load("assets/favicon/favicon1.png", &image.width,
...
void Scene::Scene::addSceneObject(
    const shared_ptr<Component::GeometryObject> &obj, bool visible, bool listed,
    bool collided, bool lighted, GLuint texture) {
...
void Scene::Scene::add(const string &name, const shared_ptr<Geometry> &geometry,
                       Transform transform, bool visible, bool listed,
                       bool collided, bool lighted, bool useBvh) {
  ...
  ptr =
      make_shared<Component::GeometryObject>(name, geometry, transform, useBvh);
  ...
  updateGeometryListView();
  auto fptr =
      find_if(this->imgui.list_items.begin(), this->imgui.list_items.end(),
              [](shared_ptr<Component::GeometryObject> obj) {
...
void Scene::Scene::addLight(const string &name,
  ...
  case Light::LightType::POINT: {
    render_obj = make_shared<Component::GeometryObject>(
        "Light", Mesh::Sphere(0.03, 72, 36));
    break;
  }
...
//       shared_ptr<vector<glm::vec3>> vec_buffer =
//       make_shared<vector<glm::vec3>>(); vec_buffer->push_back(tri_center +
//       0.005f * tri_norm);
...
glm::vec3 Scene::Scene::trace_ray(Ray ray, const HitInfo &obj, float PR,
  ...
  while (true) {
    ...
    if (new_obj.isHit) {
      ...
      RayInfo ray_info;
      ray_info.BRDF =
          glm::vec3(1.0f, 1.0f, 1.0f) / PI; // 假定理想朗伯体，完全漫反射
      ray_info.cosine =
          glm::dot(cur_ray.dir,
...
void Scene::framebufferResizeCallback(GLFWwindow *window, int width,
                                      int height) {
