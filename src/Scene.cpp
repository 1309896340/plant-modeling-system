...
Scene::Scene::Scene() {
  ...
  // init_lsystem();    // 需要在主文件中手动调用否则默认不显示
          ...
          std::visit(
            [name, this, arg_val](auto&& arg) {
              ...
              if constexpr (is_same_v<T, float>) {
                ...
                if (ImGui::DragFloat(name.c_str(), &arg, step, min_v, max_v)) {
  ...
  if (this->isShowLsystem) {
    ...
    if (ImGui::TreeNodeEx(TEXT("生成"), ImGuiTreeNodeFlags_DefaultOpen)) {
      ...
      if (ImGui::Button(TEXT("迭代"))) {
        ...
        this->lsystem.skeleton = gs.construct();   // 智能指针自动释放原对象
...
void Scene::Scene::loadIcon() {
  GLFWimage image;
  image.pixels = stbi_load("assets/favicon/favicon1.png", &image.width, &image.height, nullptr, 4);
...
void Scene::Scene::addSceneObject(
  const shared_ptr<Component::GeometryObject>& obj,
  bool                                         visible,
  bool                                         listed,
  bool                                         collided,
  bool                                         lighted,
...
void Scene::Scene::add(const string& name, const shared_ptr<Geometry>& geometry, Transform transform, bool visible, bool listed, bool collided, bool lighted, bool useBvh) {
  ...
  updateGeometryListView();
  auto fptr =
    find_if(this->imgui.list_items.begin(), this->imgui.list_items.end(), [](shared_ptr<Component::GeometryObject> obj) {
