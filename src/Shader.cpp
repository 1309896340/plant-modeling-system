...
GLuint Shader::createShader(GLuint shader_type, const std::string &source) {
  ...
  if (!success) {
    std::cout << "failed to compile " << shaderType2str(shader_type)
              << std::endl;
#ifndef NDEBUG
    char log[200] = {0};
    int log_num = 0;
...
Shader::Shader(const std::string &vertex_filename,
  ...
  GLuint vshader =
      createShader(prefix + vertex_filename);
  GLuint fshader = createShader( prefix + fragment_filename);
...
Shader::~Shader() { glDeleteProgram(this->program_id); }
...
void Shader::use() { glUseProgram(this->program_id); }

GLuint Shader::program() const { return this->program_id; }
